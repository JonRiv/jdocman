...
/*global define, jIO, btoa, b64_hmac_sha1, jQuery, XMLHttpRequest, XHRwrapper,
  FormData*/
/**
 * JIO Drupal Storage. Type = "drupal".
 * Drupal REST file entity storage.
 */
  ...
  jIO.addStorage("drupal", function (spec) {
    ...
    */
    ...
    function buildSettings(c,m){
      ...
      setObj.global_id = setObj.data._id;
        ...
        if (priv.privateserver && priv.token === ''){
          //if a private drupal server and the token is not set, grab the token
            if (priv.username && priv.password){
              ...
              xhr.open('POST', priv.server + '/?q=' + priv.endpoint + '/user/login', true);
    ...
    function postMethodWrapper(o){
      return new Promise (function(resolve, reject){
        ...
        xhr.onreadystatechange = function(){
          ...
          if (this.readyState == 4){
            switch (settings.state) {
              ...
              break;
              case 'that.put:initial post to get file id' :
                if (this.status == 200){
                  //store the current file index
                  settings.fid = JSON.parse(this.responseText).fid;
                  //set the final file index
              ...
              break;
              case 'that.put:post updated file index document' :
                if (settings.data.fid == JSON.parse(this.response).fid){
                  settings.command.success(this.status, {id: settings.data._id});
                } else {
                  return settings.command.error(
        ...
        xhr.open('POST', priv.server + '/?q=' + priv.endpoint + '/file', true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.setRequestHeader('Accept', 'application/json');
    ...
    function getMethodWrapper (o){
      return new Promise(function(resolve,reject){
        ...
        xhr.onreadystatechange = function(){
          var settings = curObj;
          if (this.readyState == 4){
            //switch to map the calling method with the request response
            switch (settings.state){
              ...
              break;
              case 'that.put:get request to check if document exists':
                if (this.status == 200){
                  //store the current file index
                  settings.fid = JSON.parse(this.responseText).fid;
                  resolve(settings);
              ...
              case 'that.get:get the document':
                if (this.status == 200){
                  settings.command.success(this.status, {'data': JSON.parse(this.responseText)});
                } else {
                  return settings.command.error(
                  404,
                  "Document not found",
              ...
              break;
              case 'that.alldocs : get json from Drupal':
                if (this.status == 200){
                  settings.data = this.responseText;
                  resolve(settings)
                }
              break;
              case 'that.allDocs : iterate on files urls to get document id':
                if (this.status == 200){
                  //possible closure problem right here
                  var wrapper = {};
                  wrapper._id = settings.global_id;
                  wrapper.data = this.responseText;
                  resolve(wrapper)
                }
              break;
            }
        ...
        //switch to map the calling method with the request URI format
        switch (curObj.state){
          case 'that.alldocs : get json from Drupal':
            xhr.open('GET', priv.server + '/?q=' + priv.endpoint + '/file', true);
            xhr.setRequestHeader('Accept','application/json');
          ...
          break;
          default :
            xhr.open('GET', priv.server + '/sites/default/files/'+curObj.file_name, true);
          break;
        }
        xhr.send();
      })
          ...
          xhr.onreadystatechange = function(){
            ...
            if (this.readyState == 4){
              switch (settings.state){
                ...
                break;
                case 'that.put:delete the original document' :
                  if (this.status == 200){
                    resolve(settings)
                  } else {
                    return settings.command.error(
                ...
                break;
                case 'that.put:delete temp fid document' :
                  if (this.status == 200){
                    resolve(settings)
                  } else {
                    return settings.command.error(
    ...
    **/
    ...
    that.post = function (command, metadata) {
      ...
      //Promise chain
      getToken(o)
        //catch in case the token was already set in the cookie
      .catch(function(result){
        return getToken(result);
      })
        //check if the document exists
      .then(function(result){
        result.state = 'that.post:get request to check if document exists';
      ...
      .then(function(result){
        result.state = 'that.post:second post with updated metadata document';
        return postMethodWrapper(result);
      })
    };
    ...
    **/
    ...
    that.put = function (command, metadata) {
      ...
      .catch(function(result){
        ...
        //check if the document exists
      .then(function(result){
        result.state = 'that.put:get request to check if document exists';
        return getMethodWrapper(result);
      })
      .then(function(result){
        result.state = 'that.put:delete the original document';
        return deleteMethodWrapper(result);
      })
        //post a document to get the file index (fid) callback
      .then(function(result){
        result.state = 'that.put:initial post to get file id';
        return postMethodWrapper(result);
      })
        //delete the document before re-posting the updated file index (fid) metadata document
      .then(function(result){
        result.state = 'that.put:delete temp fid document';
        return deleteMethodWrapper(result);
      })
        //post the updated file index (fid) metadata document
      .then(function(result){
        result.state = 'that.put:post updated file index document';
        return postMethodWrapper(result);
      })
    };
    ...
    **/
    ...
    that.allDocs = function (command, param, options) {
      var o;
      o = buildSettings(command, param, options);
      o.alldocsTemplate =       {
        "result": "success",
        "method": "allDocs",
        "id": o.data._id,
      ...
      .then(function(result){
        result.state = 'that.alldocs : get json from Drupal';
        return getMethodWrapper(result);
      })
      .then(function(result){
        result.state = 'that.allDocs : iterate on files urls to get document id';
        result.data = JSON.parse(result.data);
        var promiseArray = [];

        for (var i = 0; i < result.data.length; i++){
          result.file_name = result.data[i].filename;
          promiseArray.push(getMethodWrapper(result));
        }

        Promise.all(promiseArray)

        .then(function(results){
          o.alldocsTemplate["data"]["total_rows"] = results.length;
          for (var i = 0; i < results.length; i++){
            o.alldocsTemplate["data"]["rows"][i] = {
              "id" : JSON.parse(results[i].data)._id,
              "key": JSON.parse(results[i].data)._id,
              "value":JSON.parse(results[i].data)
            }
            if (options.include_docs == true){
                o.alldocsTemplate["data"]["rows"][i].doc = JSON.parse(results[i].data);
            }
          }
          return command.success({"data":o.alldocsTemplate["data"]});
        })
      })
    };
